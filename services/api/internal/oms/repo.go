...
const orderColumns = `
	...
	COALESCE(limit_price, 0)::float8, qty::float8, filled_qty::float8,
	COALESCE(avg_fill_price, 0)::float8, status, reject_reason, created_at, updated_at
`

func scanOrder(row pgx.Row) (*Order, error) {
	var o Order
	var limit, avg float64
	var coid, reject *string
	if err := row.Scan(
		&o.ID, &o.UserID, &coid, &o.Symbol, &o.Side, &o.Type,
		&limit, &o.Qty, &o.FilledQty, &avg, &o.Status, &reject,
...
func (r *Repo) InsertOrder(ctx context.Context, userID uuid.UUID, coid *string, symbol string, side Side, typ Type, limitPrice *float64, qty float64) (*Order, error) {
	row := r.db.QueryRow(ctx, `
		INSERT INTO orders (user_id, client_order_id, symbol, side, type, limit_price, qty, status)
		VALUES ($1, $2, $3, $4, $5, $6, $7, 'open')
		RETURNING `+orderColumns,
		userID, coid, symbol, side, typ, limitPrice, qty,
	)
...
func (r *Repo) CancelOpenOrder(ctx context.Context, userID, id uuid.UUID) (*Order, error) {
	row := r.db.QueryRow(ctx, `
		UPDATE orders
...
func (r *Repo) GetAccount(ctx context.Context, userID uuid.UUID) (*Account, error) {
	...
	err := r.db.QueryRow(ctx, `
		SELECT user_id, balance::float8, quote_currency, updated_at
		FROM accounts WHERE user_id = $1
	`, userID).Scan(&a.UserID, &a.Balance, &a.QuoteCurrency, &a.UpdatedAt)
	if errors.Is(err, pgx.ErrNoRows) {
